...
/-- `IsBayesAlgEnvSeq Q κ alg E A Y P` states that the random variable `E` has law `Q` under `P`.
It also states that, under `P`, the sequences of actions `A` and feedbacks `Y` are generated by the
algorithm `alg` interacting with the environment `stationaryEnv (κ.sectR (E ω))`. -/
